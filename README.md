...
To point it at a doc: open the doc in your browser, copy the **address-bar URL** (it ends in `/edit`), and paste it into `DOC_ID` in the script — or just the ID if you prefer. The doc must be shared **Anyone with the link → Viewer**. One gotcha: the `/pub` *viewer* link that some Google share menus offer does **not** work with the export endpoint; the sync detects it and explains, and keeps the last good content. Doc convention: start sections with Heading 2 — `Announcement` and/or `Contact`, plus an optional `Words` section (one word per line) that feeds the rotating hero word — "Illinois builds *stories* you can play." cycles the list every ~35s (built-in defaults apply until a doc list arrives). If a sync ever fails or parses to zero sections (e.g. sharing got revoked), the last good content is kept and the site is unaffected.
